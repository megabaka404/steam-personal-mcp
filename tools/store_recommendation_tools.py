...
def register_store_recommendation_tools(mcp, runtime) -> None:
    def recommend_store_for_me(count: int = 15, max_price: float | None = None, min_discount: int = 0, include_wishlist: bool = True, exclude_early_access: bool = False) -> dict[str, Any]:
    ...
    register(mcp, "recommend_store_for_me", "Retrieve unowned Store candidates with candidate_score, evidence, candidate_reasons, and potential_mismatches for an LLM to judge. candidate_score is retrieval priority, not final fit or purchase confidence. Parameters: count, max_price, min_discount, include_wishlist, exclude_early_access.", recommend_store_for_me)
